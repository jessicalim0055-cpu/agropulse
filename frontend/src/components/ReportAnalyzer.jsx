...
const DIR_ICON = {
  up:   <TrendingUp  size={13} className="text-emerald-400 flex-shrink-0 mt-0.5" />,
  down: <TrendingDown size={13} className="text-red-400   flex-shrink-0 mt-0.5" />,
  flat: <Minus        size={13} className="text-amber-400  flex-shrink-0 mt-0.5" />,
}
const DIR_COLOR = { up: 'text-emerald-400', down: 'text-red-400', flat: 'text-amber-400' }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 pt-2">
        ...
        <span className="text-sm font-semibold text-slate-300">{report.filename}</span>
      ...
      {report.overview && (
        <div className="bg-emerald-950/20 border border-emerald-800/30 rounded-xl p-5">
          ...
          <p className="text-slate-300 text-sm leading-relaxed">{report.overview}</p>
        </div>
      )}
          ...
          <div className="divide-y divide-slate-800">
            {report.week_on_week.map((w, i) => (
              <div key={i} className="py-2.5 first:pt-0 last:pb-0 flex items-start gap-3">
                ...
                <div>
                  <span className={`text-xs font-semibold ${DIR_COLOR[w.direction] || 'text-slate-400'}`}>
                    {w.commodity}
                  </span>
                  <p className="text-xs text-slate-400 mt-0.5">{w.change}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Section title="Prices">
          <div className="divide-y divide-slate-800">
            {report.prices.map((p, i) => (
              <div key={i} className="py-2.5 first:pt-0 last:pb-0 flex items-start gap-3">
                <TrendingUp size={13} className="text-emerald-500 flex-shrink-0 mt-0.5" />
                <div>
                  <span className="text-xs font-semibold text-slate-200">{p.commodity}</span>
        ...
        <Section title="Acreages & Production">
          <div className="divide-y divide-slate-800">
            {report.acreages.map((a, i) => (
              <div key={i} className="py-2.5 first:pt-0 last:pb-0 flex items-start gap-3">
                <Sprout size={13} className="text-amber-500 flex-shrink-0 mt-0.5" />
                <div>
                  <span className="text-xs font-semibold text-slate-200">{a.region}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {report.supply_demand && (
            <Section title="Supply & Demand">
              <p className="text-xs text-slate-400 leading-relaxed">{report.supply_demand}</p>
            </Section>
          )}
          {report.trade_flows && (
            <Section title="Trade Flows">
              <p className="text-xs text-slate-400 leading-relaxed">{report.trade_flows}</p>
            </Section>
          )}
        </div>
      )}
      ...
      {report.outlook && (
        <div className="bg-amber-950/20 border border-amber-800/30 rounded-xl p-5">
          ...
          <p className="text-slate-300 text-sm leading-relaxed">{report.outlook}</p>
        </div>
      )}
...
export default function ReportAnalyzer() {
  ...
  const adminPassword = sessionStorage.getItem(SESSION_KEY) || ''
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-slate-200">Market Reports</h2>
        ...
        {isAdmin ? (
          <button
            onClick={handleLock}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-700/40 bg-emerald-900/20 text-emerald-400 text-xs font-semibold hover:bg-emerald-900/40 transition-all flex-shrink-0"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-800/50 text-slate-500 text-xs font-semibold hover:text-slate-300 transition-all flex-shrink-0"
          >
            <Lock size={12} /> Admin
          </button>
        )}
      ...
      {showPw && !isAdmin && (
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-semibold rounded-lg transition-colors"
          >
            Unlock
          </button>
          {pwError && <span className="text-red-400 text-xs">Wrong password</span>}
        </div>
      )}
      ...
      {isAdmin && (
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-12 text-center transition-all
            ${dragging ? 'border-emerald-500 bg-emerald-950/30' : 'border-slate-700 hover:border-slate-600 bg-slate-900/50'}
        ...
        >
          ...
          {loading ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-slate-400 text-sm">Analysing report…</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3">
              <Upload size={28} className="text-slate-500" />
              <div>
                <p className="text-slate-300 font-medium text-sm">Drop your weekly report here</p>
      ...
      {error && (
        <div className="flex items-start gap-2 bg-red-950/30 border border-red-800/40 rounded-xl px-4 py-3 text-red-400 text-sm">
