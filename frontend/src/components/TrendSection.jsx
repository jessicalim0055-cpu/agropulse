...
const CustomTooltip = ({ active, payload, label, trends, isDark }) => {
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-xs shadow-lg">
      ...
      {payload.map(p => (
        <div key={p.dataKey} className="flex justify-between gap-6 mb-0.5">
          ...
          <span className={p.value > 0 ? 'text-emerald-600 dark:text-emerald-400' : p.value < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-500'}>
...
export default function TrendSection({ trends, period, onPeriodChange, isDark }) {
  ...
  const refLineColor = isDark ? '#334155' : '#cbd5e1'
  const refLabelFill = isDark ? '#475569' : '#94a3b8'

  return (
    <section>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-200">Sentiment Trends</h2>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium transition-all
                ${period === p.value
                  ...
                  : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700'}`}
            >
              {p.label}
            </button>
          ))}
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              ...
              />
              <ReferenceLine y={0} stroke={refLineColor} strokeDasharray="4 4" label={{ value: 'Neutral', fill: refLabelFill, fontSize: 10, position: 'right' }} />
              <Tooltip content={<CustomTooltip trends={trends} isDark={isDark} />} />
