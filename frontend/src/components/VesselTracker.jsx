...
const STATUS_STYLE = {
  'Laden':       'bg-blue-900/20 border-blue-700/40 text-blue-300',
  'Ballast':     'bg-slate-800 border-slate-700 text-slate-400',
  'Loading':     'bg-amber-900/20 border-amber-700/40 text-amber-300',
  'Discharging': 'bg-violet-900/20 border-violet-700/40 text-violet-300',
  'At Anchor':   'bg-slate-800 border-slate-700 text-slate-500',
}
...
function EtaBadge({ date, label = 'ETA' }) {
  ...
  const urgent = days <= 3
  return (
    <span className={`flex items-center gap-1 text-[10px] font-bold ${urgent ? 'text-amber-300' : 'text-slate-400'}`}>
      {urgent && <AlertTriangle size={10} />}{label} {days}d
    </span>
  )
}

// Ship icon: pointed teardrop rotated to course (0° = north)
function makeIcon(L, color, course, isSelected, isLive) {
  const deg   = (course ?? 0)
  const alpha = isLive ? 'ff' : '66'
  const c     = color + alpha.slice(0, 2)
...
export default function VesselTracker() {
  ...
  const [selected,  setSelected]  = useState(null)   // vessel object
  ...
  // ── Redraw markers whenever fleet / livePos / selected changes ──────────────
  useEffect(() => {
    const L = window.L
  ...
  // ── Derived ───────────────────────────────────────────────────────────────────
  const visibleVessels = fleet.filter(v => {
    if (v.type !== activeTab) return false
    if (operator !== 'ALL' && v.operator !== operator) return false
    return true
  })
  const opCounts = Object.fromEntries(OPERATORS.map(op => [
    op,
    op === 'ALL'
      ? fleet.filter(v => v.type === activeTab).length
      : fleet.filter(v => v.type === activeTab && v.operator === op).length,
  ]))
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-200 flex items-center gap-2">
            <Ship size={16} className="text-emerald-400" /> Vessel Tracker
          </h2>
          <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-2">
            ...
            {liveCount > 0 ? (
              <span className="flex items-center gap-1 text-emerald-400">
                ...
                {liveAge && <span className="text-slate-600">· {liveAge.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
              </span>
            ) : (
        ...
        <div className="flex items-center gap-2">
          <button onClick={fetchLive} disabled={fetching}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200 hover:border-slate-500 disabled:opacity-40 transition-all"
          >
          ...
          <a href="https://www.vesselfinder.com" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200 hover:border-slate-500 transition-all"
          ><ExternalLink size={11} /> VesselFinder</a>
          {isAdmin ? (
            <button onClick={logout}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-emerald-700/50 bg-emerald-900/20 text-emerald-400 hover:bg-emerald-900/40 transition-all"
            ><Unlock size={12} /> Admin</button>
          ) : (
            <button onClick={() => setShowLogin(v => !v)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-slate-700 text-slate-500 hover:text-slate-300 hover:border-slate-600 transition-all"
      ...
      {showLogin && !isAdmin && (
        <div className="flex items-center gap-3 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3">
          <input type="password" value={pwInput}
            ...
            className={`bg-slate-800 border rounded-lg px-3 py-1.5 text-xs text-slate-200 placeholder-slate-600 outline-none focus:border-emerald-600 w-44 ${pwError ? 'border-red-600' : 'border-slate-700'}`}
          />
          <button onClick={login} className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-600 rounded-lg text-xs text-white font-medium transition-all">Unlock</button>
          {pwError && <span className="text-xs text-red-400">Incorrect password</span>}
        </div>
      )}

      {/* ── Map ────────────────────────────────────────────────────────────── */}
      <div className="rounded-xl overflow-hidden border border-slate-800 relative" style={{ height: 380 }}>
        <div ref={mapRef} className="absolute inset-0" />
        {/* Legend */}
        <div className="absolute bottom-3 left-3 flex flex-wrap gap-2 z-[1000]">
          {Object.entries(OP_COLORS).map(([op, color]) => (
            <span key={op} className="flex items-center gap-1.5 px-2 py-1 bg-slate-900/85 rounded-lg text-[10px] font-bold backdrop-blur-sm border border-slate-700/50" style={{ color }}>
              <span className="w-2 h-2 rounded-full" style={{ background: color }} />{op}
            </span>
          ))}
        </div>
        {liveCount > 0 && (
          <div className="absolute top-3 left-3 z-[1000] flex items-center gap-1.5 px-2 py-1 bg-slate-900/85 rounded-lg text-[10px] text-emerald-400 font-semibold backdrop-blur-sm border border-emerald-800/40">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Live AIS
          </div>
        )}
      </div>
      ...
      <div className="flex gap-4" style={{ minHeight: 360 }}>
        ...
        <div className="w-44 flex-shrink-0 flex flex-col gap-1.5">
          ...
          {OPERATORS.map(op => (
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${operator === op ? 'bg-emerald-800/50 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>
                {opCounts[op] ?? 0}
              </span>
            </button>
          ))}
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0">
          ...
          <div className="flex items-center justify-between">
            ...
            {isAdmin && (
              <button onClick={openAdd}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-emerald-700/20 hover:bg-emerald-700/40 border border-emerald-700/50 text-emerald-300 rounded-lg transition-all"
          ...
          {selected && (
            ...
            >
              <button onClick={() => setSelected(null)} className="absolute top-3 right-3 text-slate-500 hover:text-slate-300 transition-colors text-lg leading-none">×</button>
              ...
              <div className="flex items-start gap-4 mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {selLive?.lat && (
                      <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-semibold bg-emerald-900/30 border border-emerald-700/40 px-1.5 py-0.5 rounded-full">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                ...
                <div className="bg-slate-900/60 rounded-lg p-2.5 border border-slate-800">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-600 mb-1">Position</p>
                  {selLive?.lat ? (
                    <p className="text-xs text-slate-300 font-mono">{selLive.lat.toFixed(3)}°, {selLive.lng.toFixed(3)}°</p>
                  ) : (
                ...
                {/* Speed & Course */}
                <div className="bg-slate-900/60 rounded-lg p-2.5 border border-slate-800">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-600 mb-1">Speed / Course</p>
                  {selLive?.speed != null ? (
                    <div className="flex items-center gap-1.5">
                      <Navigation size={11} className="text-emerald-400" style={{ transform: `rotate(${(selLive.course || 0) - 45}deg)` }} />
                      <span className="text-xs text-slate-300">{selLive.speed} kn · {Math.round(selLive.course ?? 0)}°</span>
                    </div>
                  ) : <p className="text-xs text-slate-600">—</p>}
                </div>
                {/* Nav Status */}
                <div className="bg-slate-900/60 rounded-lg p-2.5 border border-slate-800">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-600 mb-1">Nav Status</p>
                  <p className="text-xs text-slate-300">{selLive?.nav_status ?? '—'}</p>
                </div>
                {/* AIS Destination */}
                <div className="bg-slate-900/60 rounded-lg p-2.5 border border-slate-800">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-600 mb-1">AIS Destination</p>
                  <p className="text-xs text-slate-300 truncate">{selLive?.destination || selected.destination || '—'}</p>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                <div>
                  <span className="text-slate-600 text-[10px] uppercase tracking-wider mr-1">Route</span>
                  <span className="text-slate-300">{selected.origin} → {selected.destination}</span>
                </div>
                {selected.etd && (
                  <div>
                    <span className="text-slate-600 text-[10px] uppercase tracking-wider mr-1">ETD</span>
                    <span className="text-slate-300">{fmtDate(selected.etd)}</span>
                    <span className="ml-1"><EtaBadge date={selected.etd} label="dep" /></span>
                  </div>
                )}
                {selected.eta && (
                  <div>
                    <span className="text-slate-600 text-[10px] uppercase tracking-wider mr-1">ETA</span>
                    <span className="text-slate-300">{fmtDate(selected.eta)}</span>
                    <span className="ml-1"><EtaBadge date={selected.eta} label="arr" /></span>
                  </div>
                )}
                {selected.imo && (
            ...
            <table className="w-full">
              ...
              <tbody>
                {visibleVessels.length === 0 && (
                  <tr><td colSpan={isAdmin ? 7 : 6} className="text-center text-xs text-slate-600 py-10">No vessels found.</td></tr>
                )}
                {visibleVessels.map(v => {
                  ...
                  return (
                    <tr key={v.id} onClick={() => focusVessel(v)}
                      className={`border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors last:border-0 cursor-pointer
                        ${isSel ? 'bg-slate-800/50 ring-1 ring-inset ring-slate-600' : ''}`}
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500">{v.imo ? `IMO ${v.imo}` : <span className="text-slate-700">No IMO</span>}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <p className="text-xs text-slate-300">{v.origin}</p>
                        <p className="text-[10px] text-slate-500">→ {v.destination}</p>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-xs text-slate-400">{fmtDate(v.etd)}</p>
                        <EtaBadge date={v.etd} label="dep" />
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-xs text-slate-400">{fmtDate(v.eta)}</p>
                      ...
                      {isAdmin && (
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => openEdit(v)} className="p-1 text-slate-500 hover:text-slate-300 transition-colors"><Edit3 size={12} /></button>
                            <button onClick={() => deleteVessel(v.id)} className="p-1 text-slate-500 hover:text-red-400 transition-colors"><Trash2 size={12} /></button>
                          </div>
                        </td>
                      )}
                    </tr>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
              <p className="text-sm font-bold text-slate-200">{editId ? 'Edit Vessel' : 'Add Vessel'}</p>
              <button onClick={closeForm} className="text-slate-500 hover:text-slate-300 transition-colors"><X size={16} /></button>
            </div>
            <div className="p-5 space-y-4">

              {/* IMO field with auto-lookup */}
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    maxLength={8}
                    className="flex-1 bg-slate-800 border border-emerald-800/50 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-600 outline-none focus:border-emerald-500 transition-colors font-mono"
                  />
                  <button
                    onClick={lookupIMO}
                    disabled={!form.imo || looking}
                    className="flex items-center gap-1.5 px-3 py-2 bg-emerald-700/20 hover:bg-emerald-700/40 border border-emerald-700/50 text-emerald-300 rounded-lg text-xs font-medium disabled:opacity-40 transition-all"
                  >
                    <Search size={12} className={looking ? 'animate-spin' : ''} />
                    {looking ? 'Looking up…' : 'Auto-fill'}
                  </button>
                </div>
                <p className="text-[10px] text-slate-600 mt-1">Find the IMO on <a href="https://www.vesselfinder.com" target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline">VesselFinder</a> by searching the vessel name.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input value={form.name || ''} onChange={e => setField('name', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-600 transition-colors"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={form.status || 'Laden'} onChange={e => setField('status', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-600"
                  >{Object.keys(STATUS_STYLE).map(s => <option key={s}>{s}</option>)}</select>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 mb-1">Operator</p>
                  <select value={form.operator || 'PEIRU'} onChange={e => setField('operator', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-600"
                  >{OPERATORS.filter(op => op !== 'ALL').map(op => <option key={op}>{op}</option>)}</select>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 mb-1">Type</p>
                  <select value={form.type || 'bulk'} onChange={e => setField('type', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-600"
                  >
              ...
              <details className="text-[10px]">
                <summary className="text-slate-600 cursor-pointer hover:text-slate-400 transition-colors">Fallback position (used if IMO not found)</summary>
                <div className="grid grid-cols-2 gap-3 mt-2">
                  ...
                  <div>
                    ...
                    <input type="number" value={form.lng ?? ''} onChange={e => setField('lng', e.target.value)}
                      className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-600"
                    />
                  </div>
                </div>
            ...
            <div className="flex gap-2 px-5 pb-5">
              <button onClick={saveForm} className="flex-1 py-2 bg-emerald-700 hover:bg-emerald-600 rounded-lg text-xs text-white font-semibold transition-all">
                {editId ? 'Save Changes' : 'Add Vessel'}
              </button>
              <button onClick={closeForm} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs text-slate-400 transition-all">Cancel</button>
            </div>
          </div>
        </div>
      )}
